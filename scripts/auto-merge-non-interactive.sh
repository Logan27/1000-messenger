...
# Non-interactive auto-merge script for merging all feature branches to main
...
FEATURE_BRANCHES=$(git branch -r | grep -v 'origin/main' | grep -v 'origin/HEAD' | sed 's/origin\///' | grep -v '^[[:space:]]*$')
...
merge_branch() {
    ...
    # Attempt to merge
    if git merge --no-ff "origin/$branch" -m "Auto-merge branch $branch"; then
...
if [ $FAILED_COUNT -gt 0 ]; then
    ...
    # Delete the backup branch automatically since all merges succeeded
    git branch -D "$BACKUP_BRANCH"
    print_success "Backup branch deleted automatically"
fi
